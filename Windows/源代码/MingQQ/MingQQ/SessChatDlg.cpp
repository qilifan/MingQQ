...
void CSessChatDlg::OnRecvMsg(UINT nQQUin, UINT nMsgId)
{
	...
	if (nMsgId == 0)
	{
		...
		if (lpMsgList != NULL)
		{
			CMessageSender * lpMsgSender = lpMsgList->GetMsgSender(QQ_MSG_TYPE_SESS, nQQUin);
			if (lpMsgSender != NULL)
			{
				...
				lpMsgList->DelMsgSender(QQ_MSG_TYPE_SESS, nQQUin);
				::PostMessage(m_hMainDlg, WM_DEL_MSG_SENDER, QQ_MSG_TYPE_SESS, nQQUin);
			}
		}
	}
	else
...
void CSessChatDlg::OnUpdateGMemberNumber()
{
	...
	if (m_lpQQClient->IsNeedUpdateSessHeadPic(m_nQQNumber))	// 更新头像
		m_lpQQClient->UpdateGroupMemberHeadPic(m_nGroupCode, m_nQQUin, m_nQQNumber);
}
...
void CSessChatDlg::OnUpdateGMemberHeadPic()
{
	...
	if (m_lpQQClient != NULL && m_nQQNumber != 0)
		strFileName = m_lpQQClient->GetSessHeadPicFullName(m_nQQNumber);
...
BOOL CSessChatDlg::OnInitDialog(CWindow wndFocus, LPARAM lInitParam)
{
	...
	if (lpBuddyInfo != NULL)
	{
		if (!lpBuddyInfo->IsHasQQNum())		// 更新好友号码
		{
			m_lpQQClient->UpdateGroupMemberNum(m_nGroupCode, m_nQQUin);
		}
		else								// 更新好友头像
		{
			if (m_lpQQClient->IsNeedUpdateSessHeadPic(lpBuddyInfo->m_nQQNum))
				m_lpQQClient->UpdateGroupMemberHeadPic(m_nGroupCode, lpBuddyInfo->m_nQQUin, lpBuddyInfo->m_nQQNum);
		}
	}
...
{
	//::PostMessage(m_hMainDlg, WM_CLOSE_SESSCHATDLG, m_nGroupCode, m_nQQUin);
	DestroyWindow();
}
...
{
	::PostMessage(m_hMainDlg, WM_SHOW_GMEMBERINFODLG, m_nGroupCode, m_nQQUin);
}
...
void CSessChatDlg::OnBtn_Send(UINT uNotifyCode, int nID, CWindow wndCtl)
{
	...
	m_lpQQClient->SendSessMsg(m_nGroupId, m_nQQUin, nTime, strText.c_str());
...
CBuddyInfo * CSessChatDlg::GetBuddyInfoPtr()
{
	if (m_lpQQClient != NULL)
	{
		...
		if (lpGroupInfo != NULL)
			return lpGroupInfo->GetMemberByUin(m_nQQUin);
	}
	return NULL;
}
...
void CSessChatDlg::UpdateData()
{
	...
	if (lpBuddyInfo != NULL)
	{
		...
		if (!lpBuddyInfo->m_strGroupCard.empty())
			m_strBuddyName = lpBuddyInfo->m_strGroupCard.c_str();
		else if (!lpBuddyInfo->m_strMarkName.empty())
...
BOOL CSessChatDlg::Init()
{
	...
	m_staGroupName.MoveWindow(44, 22, rcClient.right-2, 14, FALSE);
...
{
	if (NULL == lpSessMsg || NULL == m_lpQQClient)
		return;
...
void CSessChatDlg::OpenMsgLogBrowser()
{
	...
	strCmdLine.Format(_T("\"%s\" %u %u %u %u %u \"%s\" \"%s\""), 
		strExeName, m_hWnd, 0, 0, m_nQQNumber, m_nUserNumber, strMsgFile, strChatPicDir);
